...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/latest%20tag-wallet--v0.3.5-7a4a22?style=flat-square" alt="Latest tag: wallet-v0.3.5">
...
The `main-v0.4` branch is the principal line and the source of `wallet-v0.3.5`. It integrates, on top of `wallet-v0.3.2`:
...
It derives network hashrate from the last 30 canonical block headers only when synchronized, exposes the network and local rates separately, and presents the full DEPC-3 uncertainty range. Swap inputs now use each asset's real base unit and calculate the protocol fee only from the DOM leg supplied by an accepted solver quote.
...
The principal development and release line for `wallet-v0.3.5`. It merges into one branch, on top of `wallet-v0.3.2`:
