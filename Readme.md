...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/latest%20tag-wallet--v0.3.4-7a4a22?style=flat-square" alt="Latest tag: wallet-v0.3.4">
...
The `main-v0.4` branch is the principal line and the source of `wallet-v0.3.4`. It integrates, on top of `wallet-v0.3.2`:
...
The authoritative per-version record — artifacts, checksums, and release notes from `wallet-v0.1.0` through `wallet-v0.3.4` — is the repository's GitHub Releases page.

### wallet-v0.3.4

Latest release from the `main-v0.4` line. It derives network hashrate from the last 30 canonical block headers only when synchronized, exposes the network and local rates separately, and presents the full DEPC-3 uncertainty range. Swap inputs now use each asset's real base unit and calculate the protocol fee only from the DOM leg supplied by an accepted solver quote.

### wallet-v0.3.3

It adds the DEPC-3 mining estimate, fail-closed fee presentation, seed-derived multichain accounts, the resumable Swap interface and protocol client, kernel-evidence confirmation, restart-safe persistence, and remote-input and secret-handling hardening. Swap execution remains fail-closed unless a compatible interop daemon is available.
...
The principal development and release line for `wallet-v0.3.4`. It merges into one branch, on top of `wallet-v0.3.2`:
