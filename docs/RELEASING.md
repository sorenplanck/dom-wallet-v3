...
- Wallet version: `0.3.6`
- Recommended tag: `wallet-v0.3.6`
...
`validation_only=false` and `release_version=0.3.6`. This builds unsigned Linux,
...
that the clean release commit reports version `0.3.6`, then run:

```bash
git tag -a wallet-v0.3.6 -m "DOM Wallet V3 0.3.6 experimental"
git push origin wallet-v0.3.6
```
